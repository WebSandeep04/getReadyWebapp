    ...
    <div class="row g-3 mb-4">
        <div class="col-lg-3 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-card__value" id="totalCities">{{ number_format($total ?? 0) }}</div>
                <small>Urban destinations</small>
            </div>
        </div>
    </div>
    ...
    <div class="card h-100">
        <div class="card-header bg-white text-dark border-bottom py-3">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                <h5 class="mb-0 fw-bold"><i class="bi bi-table me-2"></i>City Management</h5>
                <div class="d-flex flex-wrap gap-2">
                    <div class="input-group input-group-sm" style="width: 200px;">
                        <span class="input-group-text bg-white border-end-0 text-muted"><i class="bi bi-filter"></i></span>
                        <select id="stateFilter" class="form-select border-start-0 ps-0">
                    ...
                    <div class="input-group input-group-sm" style="width: 200px;">
                        ...
                        <input type="search" id="citySearch" class="form-control border-start-0 ps-0" placeholder="Search cities...">
                    </div>
                    <button class="btn btn-sm btn-dark" id="addCityBtn">
                        <i class="bi bi-plus-lg me-1"></i>Add New
                    </button>
                    <button class="btn btn-sm btn-outline-dark" id="refreshCities" title="Refresh">
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0 align-middle admin-table" id="citiesTable">
            ...
            <!-- Pagination -->
            <div class="card-footer bg-white border-top border-light py-3 px-4 d-none" id="citiesPagination"></div>
            
            <!-- Empty State -->
            <div class="text-center py-5 d-none" id="citiesEmptyState">
                <div class="mb-3 text-muted opacity-25">
                    <i class="bi bi-search" style="font-size: 3rem;"></i>
                </div>
                <h6 class="fw-bold">No cities match that filter</h6>
                <p class="text-muted small mb-3">Try clearing the search or filter to view all cities.</p>
                <button class="btn btn-sm btn-dark" id="clearCityFilters">Reset filters</button>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header border-bottom-0 pb-0">
                    <h5 class="modal-title fw-bold" id="cityModalLabel">Add City</h5>
                ...
                <form id="cityForm">
                    <div class="modal-body pt-4">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="cityState" name="state_id" required>
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-bold small text-uppercase text-muted">City Name</label>
                            <input type="text" class="form-control" id="cityName" name="name" required placeholder="e.g. Mumbai">
                        </div>
                        <div class="mb-3">
                    ...
                    <div class="modal-footer border-top-0">
                        ...
                        <button type="submit" class="btn btn-sm btn-dark px-4">Save City</button>
                    </div>
                </form>
            </div>
        </div>
...
$(function() {
    ...
    const endpoints = {
        fetch: "{{ route('cities.json') }}",
        store: "{{ route('cities.store') }}",
        update: "{{ url('/admin/cities') }}",
        delete: "{{ url('/admin/cities') }}",
        toggle: "{{ url('/admin/cities/toggle') }}",
    };

    const $tableBody = $('#citiesTable tbody');
    const $emptyState = $('#citiesEmptyState');
    const $search = $('#citySearch');
    const $stateFilter = $('#stateFilter');
    const $refresh = $('#refreshCities');
    ...
    function fetchCities(showSpinner = true) {
        if (showSpinner) {
            $refresh.prop('disabled', true).html('<span class="spinner-border spinner-border-sm"></span>');
        }
        $.getJSON(endpoints.fetch, { state_id: state.stateId })
            .done(data => {
                state.data = data || [];
                state.page = 1;
                renderCities();
    ...
    function updateStats() {
        ...
        $('#totalCities').text(new Intl.NumberFormat().format(total));
    }
    ...
    function renderCities() {
        ...
        if (!filtered.length) {
            ...
            $('#citiesPagination').addClass('d-none');
    ...
    function renderRow(item) {
        ...
        const btns = `
            <div class="btn-group" role="group">
                <button class="btn btn-sm btn-outline-secondary edit-city" data-item='${JSON.stringify(item)}' title="Edit">
                    <i class="bi bi-pencil"></i>
                </button>
                <button class="btn btn-sm btn-outline-danger delete-city" data-id="${item.id}" title="Delete">
        ...
        return `
            <tr>
                ...
                <td><span class="text-muted">${item.state ? item.state.name : 'N/A'}</span></td>
                <td>${statusBadge}</td>
    ...
    $stateFilter.on('change', function() {
        state.stateId = $(this).val();
        state.page = 1;
        fetchCities();
    });

    $('#clearCityFilters').on('click', function() {
        state.search = '';
        state.stateId = '';
        state.page = 1;
        $search.val('');
    ...
    $tableBody.on('click', '.edit-city', function() {
        const item = $(this).data('item');
        $('#cityId').val(item.id);
        $('#cityState').val(item.state_id);
    ...
    $tableBody.on('click', '.delete-city', function() {
        const id = $(this).data('id');
        ...
        $.ajax({
          ...
          .fail(() => alert('Unable to delete city.'));
    });
