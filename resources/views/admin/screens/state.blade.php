...
<div class="container-fluid p-0">
    ...
    <div class="row g-3 mb-4">
        <div class="col-lg-3 col-sm-6">
            <div class="stat-card">
                <div class="stat-card__icon"><i class="bi bi-map"></i></div>
                <div class="stat-card__label">Total States</div>
                <div class="stat-card__value" id="totalStates">{{ number_format($total ?? 0) }}</div>
                <small>Geographical regions</small>
            </div>
        </div>
    </div>
    ...
    <div class="card h-100">
        <div class="card-header bg-white text-dark border-bottom py-3">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="mb-0 fw-bold"><i class="bi bi-table me-2"></i>State Management</h5>
                <div class="d-flex gap-2">
                     <div class="input-group input-group-sm" style="width: 250px;">
                        <span class="input-group-text bg-white border-end-0 text-muted"><i class="bi bi-search"></i></span>
                        <input type="search" id="stateSearch" class="form-control border-start-0 ps-0" placeholder="Search states...">
                    </div>
                    <button class="btn btn-sm btn-dark" id="addStateBtn">
                        <i class="bi bi-plus-lg me-1"></i>Add New
                    </button>
                    <button class="btn btn-sm btn-outline-dark" id="refreshStates" title="Refresh">
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0 align-middle admin-table" id="statesTable">
            ...
            <!-- Pagination -->
            <div class="card-footer bg-white border-top border-light py-3 px-4 d-none" id="statesPagination"></div>
            
            <!-- Empty State -->
            <div class="text-center py-5 d-none" id="statesEmptyState">
                <div class="mb-3 text-muted opacity-25">
                    <i class="bi bi-search" style="font-size: 3rem;"></i>
                </div>
                <h6 class="fw-bold">No states match that filter</h6>
                <p class="text-muted small mb-3">Try clearing the search to view all states.</p>
                <button class="btn btn-sm btn-dark" id="clearStateFilters">Reset filters</button>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header border-bottom-0 pb-0">
                    <h5 class="modal-title fw-bold" id="stateModalLabel">Add State</h5>
                ...
                <form id="stateForm">
                    <div class="modal-body pt-4">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="stateName" name="name" required placeholder="e.g. Maharashtra">
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold small text-uppercase text-muted">Status</label>
                            <select class="form-select" id="stateStatus" name="status">
                    ...
                    <div class="modal-footer border-top-0">
                        ...
                        <button type="submit" class="btn btn-sm btn-dark px-4">Save State</button>
                    </div>
                </form>
            </div>
        </div>
...
$(function() {
    ...
    const endpoints = {
        fetch: "{{ route('states.json') }}",
        store: "{{ route('states.store') }}",
        update: "{{ url('/admin/states') }}",
        delete: "{{ url('/admin/states') }}",
        toggle: "{{ url('/admin/states/toggle') }}",
    };

    const $tableBody = $('#statesTable tbody');
    const $emptyState = $('#statesEmptyState');
    const $search = $('#stateSearch');
    const $refresh = $('#refreshStates');
    ...
    function fetchStates(showSpinner = true) {
        if (showSpinner) {
    ...
    function updateStats() {
        ...
        $('#totalStates').text(new Intl.NumberFormat().format(total));
    }
    ...
    function renderStates() {
        ...
        if (!filtered.length) {
            $('#statesTable').addClass('d-none');
            $('#statesPagination').addClass('d-none');
            $emptyState.removeClass('d-none');
            return;
        }

        $('#statesTable').removeClass('d-none');
    ...
    function renderRow(item) {
        ...
        const btns = `
            <div class="btn-group" role="group">
                <button class="btn btn-sm btn-outline-secondary edit-state" data-item='${JSON.stringify(item)}' title="Edit">
                    <i class="bi bi-pencil"></i>
                </button>
                <button class="btn btn-sm btn-outline-danger delete-state" data-id="${item.id}" title="Delete">
    ...
    $('#clearStateFilters').on('click', function() {
        state.search = '';
        state.page = 1;
        $search.val('');
        renderStates();
    ...
    $tableBody.on('click', '.edit-state', function() {
        const item = $(this).data('item');
        $('#stateId').val(item.id);
        $('#stateName').val(item.name);
        $('#stateStatus').val(item.status);
    ...
    $tableBody.on('click', '.delete-state', function() {
        const id = $(this).data('id');
        ...
        $.ajax({
          ...
          .fail(() => alert('Unable to delete state.'));
    });
