    ...
    <div class="row g-3 mb-4">
        <div class="col-lg-3 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-card__label">Total Bottom Types</div>
                <div class="stat-card__value" id="totalBottomTypes">{{ number_format($total ?? 0) }}</div>
                <small>Defined silhouettes</small>
            </div>
        </div>
    </div>
    ...
    <div class="card h-100">
        <div class="card-header bg-white text-dark border-bottom py-3">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="mb-0 fw-bold"><i class="bi bi-table me-2"></i>Bottom Type Management</h5>
                <div class="d-flex gap-2">
                     <div class="input-group input-group-sm" style="width: 250px;">
                        <span class="input-group-text bg-white border-end-0 text-muted"><i class="bi bi-search"></i></span>
                        <input type="search" id="bottomTypeSearch" class="form-control border-start-0 ps-0" placeholder="Search bottom types...">
                    </div>
                    <button class="btn btn-sm btn-dark" data-bs-toggle="modal" data-bs-target="#createBottomTypeModal">
                        <i class="bi bi-plus-lg me-1"></i>Add New
                    </button>
                    <button class="btn btn-sm btn-outline-dark" id="refreshBottomTypes" title="Refresh">
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0 align-middle admin-table" id="bottomTypesTable">
            ...
            <!-- Pagination -->
            <div class="card-footer bg-white border-top border-light py-3 px-4 d-none" id="bottomTypesPagination"></div>
            
            <!-- Empty State -->
            <div class="text-center py-5 d-none" id="bottomTypesEmptyState">
                <div class="mb-3 text-muted opacity-25">
                    <i class="bi bi-search" style="font-size: 3rem;"></i>
                </div>
                <h6 class="fw-bold">No bottom types match that filter</h6>
                <p class="text-muted small mb-3">Try clearing the search to view all bottom types.</p>
                <button class="btn btn-sm btn-dark" id="clearBottomTypeFilters">Reset filters</button>
            </div>
        </div>
    </div>

    <!-- Create/Edit Modal -->
    <div class="modal fade" id="bottomTypeModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header border-bottom-0 pb-0">
                    <h5 class="modal-title fw-bold" id="bottomTypeModalLabel">Add Bottom Type</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form id="bottomTypeForm">
                    <div class="modal-body pt-4">
                        <input type="hidden" id="bottomTypeId">
                        <div class="mb-3">
                            <label class="form-label fw-bold small text-uppercase text-muted">Name</label>
                            <input type="text" class="form-control" id="bottomTypeName" required placeholder="e.g. Flare">
                        </div>
                    </div>
                    <div class="modal-footer border-top-0">
                        <button type="button" class="btn btn-sm btn-outline-secondary px-3" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" class="btn btn-sm btn-dark px-4">Save Bottom Type</button>
                    </div>
                </form>
            </div>
        </div>
...
$(function() {
    ...
    const endpoints = {
        fetch: "{{ route('bottom_types.json') }}",
        store: "{{ route('bottom_types.store') }}",
        update: "{{ url('/admin/bottom-types') }}",
        delete: "{{ url('/admin/bottom-types') }}",
    };

    const $tableBody = $('#bottomTypesTable tbody');
    const $emptyState = $('#bottomTypesEmptyState');
    const $search = $('#bottomTypeSearch');
    const $refresh = $('#refreshBottomTypes');
    const bottomTypeModal = new bootstrap.Modal(document.getElementById('bottomTypeModal'));

    $('[data-bs-target="#createBottomTypeModal"]').click(function() {
        $('#bottomTypeModalLabel').text('Add Bottom Type');
        $('#bottomTypeForm')[0].reset();
        $('#bottomTypeId').val('');
        bottomTypeModal.show();
    });

    function fetchBottomTypes(showSpinner = true) {
        if (showSpinner) {
        ...
        $.getJSON(endpoints.fetch)
            ...
            .fail(() => {
                console.error('Failed to load bottom types');
            })
            .always(() => {
    ...
    function updateStats() {
        ...
        $('#totalBottomTypes').text(new Intl.NumberFormat().format(total));
    }

    function renderBottomTypes() {
        ...
        if (!filtered.length) {
            $('#bottomTypesTable').addClass('d-none');
            $('#bottomTypesPagination').addClass('d-none');
            $emptyState.removeClass('d-none');
            return;
        }

        $('#bottomTypesTable').removeClass('d-none');
    ...
    function renderPagination(totalEntries, totalPages) {
        const $pager = $('#bottomTypesPagination');
    ...
    $('#clearBottomTypeFilters').on('click', function() {
        state.search = '';
        state.page = 1;
        $search.val('');
        renderBottomTypes();
    ...
    $refresh.on('click', function() {
        fetchBottomTypes(false);
    });

    $tableBody.on('click', '.edit-item', function() {
        const item = $(this).data('item');
        $('#bottomTypeId').val(item.id);
        $('#bottomTypeName').val(item.name);
        $('#bottomTypeModalLabel').text('Edit Bottom Type');
        bottomTypeModal.show();
    });

    $('#bottomTypeForm').on('submit', function(e) {
        e.preventDefault();
        const id = $('#bottomTypeId').val();
        const isEdit = !!id;
        ...
        $.ajax({
            ...
            type: method,
            data: {
                name: $('#bottomTypeName').val(),
                _token: csrf,
            }
        })
        .done(() => {
            bottomTypeModal.hide();
            fetchBottomTypes(false);
        })
        .fail(xhr => {
            alert('Unable to save bottom type.');
        })
        .always(() => {
    ...
    $tableBody.on('click', '.delete-item', function() {
        ...
        if (!confirm('Delete this bottom type?')) return;
        
        $.ajax({
            url: `${endpoints.delete}/${id}`,
            type: 'DELETE',
            data: { _token: csrf },
        }).done(() => fetchBottomTypes(false))
          .fail(() => alert('Unable to delete bottom type.'));
    });
