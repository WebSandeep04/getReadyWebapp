...
<div class="container-fluid p-0">
    ...
    <div class="row g-3 mb-4">
        <div class="col-lg-3 col-sm-6">
            <div class="stat-card">
                <div class="stat-card__icon"><i class="bi bi-palette"></i></div>
                <div class="stat-card__label">Total Colors</div>
                <div class="stat-card__value" id="totalColors">{{ number_format($total ?? 0) }}</div>
                <small>Available shades</small>
            </div>
        </div>
    </div>
    ...
    <div class="card h-100">
        <div class="card-header bg-white text-dark border-bottom py-3">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="mb-0 fw-bold"><i class="bi bi-table me-2"></i>Color Management</h5>
                <div class="d-flex gap-2">
                     <div class="input-group input-group-sm" style="width: 250px;">
                        <span class="input-group-text bg-white border-end-0 text-muted"><i class="bi bi-search"></i></span>
                        <input type="search" id="colorSearch" class="form-control border-start-0 ps-0" placeholder="Search colors...">
                    </div>
                    <button class="btn btn-sm btn-dark" data-bs-toggle="modal" data-bs-target="#createColorModal">
                        <i class="bi bi-plus-lg me-1"></i>Add New
                    </button>
                    <button class="btn btn-sm btn-outline-dark" id="refreshColors" title="Refresh">
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0 align-middle admin-table" id="colorsTable">
            ...
            <!-- Pagination -->
            <div class="card-footer bg-white border-top border-light py-3 px-4 d-none" id="colorsPagination"></div>
            
            <!-- Empty State -->
            <div class="text-center py-5 d-none" id="colorsEmptyState">
                <div class="mb-3 text-muted opacity-25">
                    <i class="bi bi-search" style="font-size: 3rem;"></i>
                </div>
                <h6 class="fw-bold">No colors match that filter</h6>
                <p class="text-muted small mb-3">Try clearing the search to view all colors.</p>
                <button class="btn btn-sm btn-dark" id="clearColorFilters">Reset filters</button>
            </div>
        </div>
    </div>
                ...
                <form id="colorForm">
                    <div class="modal-body pt-4">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="colorName" required placeholder="e.g. Royal Blue">
                        </div>
                    </div>
                    <div class="modal-footer border-top-0">
                        <button type="button" class="btn btn-sm btn-outline-secondary px-3" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" class="btn btn-sm btn-dark px-4">Save Color</button>
                    </div>
                </form>
            </div>
        </div>
...
$(function() {
    ...
    function fetchColors(showSpinner = true) {
        if (showSpinner) {
    ...
    function updateStats() {
        ...
        $('#totalColors').text(new Intl.NumberFormat().format(total));
    }
    ...
    function renderColors() {
        ...
        if (!filtered.length) {
            $('#colorsTable').addClass('d-none');
            $('#colorsPagination').addClass('d-none');
            $emptyState.removeClass('d-none');
            return;
        }

        $('#colorsTable').removeClass('d-none');
    ...
    $('#clearColorFilters').on('click', function() {
        state.search = '';
        state.page = 1;
        $search.val('');
        renderColors();
    ...
    $tableBody.on('click', '.delete-item', function() {
        ...
        $.ajax({
            ...
            type: 'DELETE',
            data: { _token: csrf },
        }).done(() => fetchColors(false))
          .fail(() => alert('Unable to delete color.'));
    });
